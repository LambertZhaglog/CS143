 ...
 */
    ...
    */
    ...
    %type <formal> formal
    %type <expressions> exp_list_colon exp_list_semi
    %type <expression> exp let_sub_exp let
...
|     	/* empty */ 
    ...
    { $$=attr($1,$3,$5);}
    | OBJECTID '(' formal_list ')' ':' TYPEID '{' exp '}'  /* in this case, formal_list can be nil */
    { $$=method($1,$3,$6,$8);}
| error ':' TYPEID {}
| error ':' TYPEID ASSIGN exp {}
| error '(' formal_list ')' ':' TYPEID '{' exp '}' {}
    ;
...
OBJECTID ASSIGN exp
{$$=assign($1,$3);}
| exp '.' OBJECTID '(' exp_list_colon ')'
{$$=dispatch($1,$3,$5);}
| exp '@' TYPEID '.' OBJECTID '(' exp_list_colon ')'
{$$=static_dispatch($1,$3,$5,$7);}
| OBJECTID '(' exp_list_colon ')'
{$$=dispatch(object(idtable.add_string("self")),$1,$3);}
| IF exp THEN exp ELSE exp FI
{$$=cond($2,$4,$6);}
| WHILE exp LOOP exp POOL
{$$=loop($2,$4);}
| WHILE exp error exp POOL {}
| WHILE exp LOOP exp error {}
| '{' exp_list_semi '}'
{$$=block($2);}
 ...
 * This is the constructor for let expressions. Note that the let constructor only allows one identifier. When parsing a let expression with multiple identifiers, it should be transformed into nested lets with single identifiers, as described in the semantics for let in the Cool Reference Manual.
 */
let:
LET OBJECTID ':' TYPEID let_sub_exp
{$$=let($2,$4,no_expr(),$5);}
| LET OBJECTID ':' TYPEID ASSIGN exp let_sub_exp
{$$=let($2,$4,$6,$7);}
;
...
IN exp
{$$=$2;}
| ',' OBJECTID ':' TYPEID let_sub_exp
{$$=let($2,$4,no_expr(),$5);}
| ',' OBJECTID ':' TYPEID ASSIGN exp let_sub_exp
{$$=let($2,$4,$6,$7);}
;
...
exp_list_colon:// formalization [ expr [, expr]∗ ] [expr; ]+
/* empty */
{$$=nil_Expressions();}
| exp
